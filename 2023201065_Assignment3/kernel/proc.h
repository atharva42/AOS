...
struct proc {
  ...
  uint runTime, createTime, exitTime;
  int newProc, statPri, runNum, lastSched, exitSched, lastRun;
  int level, inQueue, queueChange, queueEnterTime, queue[5];
  ...
  struct trapframe *alarm_trapframe;  // trapframe cache
  int alarm_state;                    // 1 = on, 0 = off
  int ticks;
  int cur_ticks;
};
...
// int _set_priority(int new_priority, int pid);
